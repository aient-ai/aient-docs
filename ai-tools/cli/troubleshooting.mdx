...
Since Aient CLI 0.8.2, `sandbox sync` resets its timeout while upload bytes continue to flow and when the server begins responding. If a current transfer fails despite continuing progress, check `aient version`; releases before 0.8.2 applied the default five-minute budget to the whole HTTP request.

For an unusually slow connection, increase the permitted inactivity gap:

```sh
aient --timeout 20m --verbose sandbox sync laptop-offload .
```

An environment-bound customer should start a fresh `sandbox run --environment ... --keep` to transfer changed workspace state in 0.10.0.

Retrying `sync` starts a new delta or snapshot attempt. It does not continue the previous multipart upload from its last reported byte.
...
## A retained customer sandbox rejects `sync`, `files`, or `shell`

An environment-bound sandbox created with `sandbox run --environment ... --keep` requires the original authority selectors on later `exec` and `execution` operations. `status`, `wait`, and `logs` require the original `--environment` selector. In 0.10.0, `sync`, `files`, and `shell` cannot address the sandbox. Start a fresh `run --keep` when local workspace state changes.
...
- Environment-bound customer sandboxes cannot use `sync`, `files`, or `shell`.
