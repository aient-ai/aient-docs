...
Although 0.7.0 help exposes template, named-volume, CPU, and memory flags, customer environment sizing is server-owned. Remove non-default `--template`, every `--named-volume`, and `--cpu`, `--cpu-limit`, `--memory`, or `--memory-limit`. They remain low-level operator controls until customer size presets are released.

## A retained customer sandbox rejects `sync`, `shell`, or `status`

An environment-bound sandbox created with `sandbox run --environment ... --keep` requires the original authority selectors on later `exec` and `execution` operations. In 0.7.0, only those selector-bearing operations, `list --environment`, and owner `delete` support the bound customer contract. `sync`, `files`, `shell`, `status`, `wait`, and `logs` cannot address the sandbox. Start a fresh `run --keep` when local workspace state changes; do not retry with operator credentials.
