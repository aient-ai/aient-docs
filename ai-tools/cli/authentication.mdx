...
`auth status --all` displays local metadata for every profile without refreshing credentials. A status check for a selected refresh-backed profile verifies the live session; a status check using an access-only token validates its signed context and expiry locally, with server authority checked on the first API operation.
...
Export a shorter-lived access-only credential for a child process, CI job, or another machine:

```sh
eval "$(aient auth export --profile client-a --format env --ttl 10m)"
aient sandbox list
unset AIENT_ACCESS_TOKEN
```

Or create a strict JSON envelope:

```sh
aient auth export --profile client-a --format file --ttl 10m \
  --output /private/tmp/aient-access.json

aient --access-token-file /private/tmp/aient-access.json sandbox list
rm /private/tmp/aient-access.json
```
...
<Warning>
  The `env` and `json` formats deliberately print a live bearer token. Disable shell tracing, keep the output out of logs and transcripts, unset it after use, and delete exported files when the receiving process finishes.
</Warning>

An export contains no refresh token, operator key, environment secret, repository credential, or GitHub token. Imported access-only credentials are not refreshed or persisted by the CLI. When one expires, export another or sign in on that machine.
...
Operator API keys and auth environment files are not customer authentication alternatives. They require the explicitly reserved `--profile operator` and are intended only for Aient's internal operations.
