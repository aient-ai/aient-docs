...
Read the current content-free state:

```sh
aient sandbox execution status laptop-offload EXECUTION_ID
aient sandbox execution status laptop-offload EXECUTION_ID --json
```

For an environment-authorised execution, repeat the same selector context:
...
```sh
aient sandbox execution cancel laptop-offload EXECUTION_ID
aient sandbox execution cancel laptop-offload EXECUTION_ID --json
```
...
File commands address an existing sandbox and an explicit path. They are separate from exact Git workspace synchronization.
...
Without a directory argument, the server's default directory is used.
