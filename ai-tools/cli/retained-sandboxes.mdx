...
The 0.7.0 help also exposes template, named-volume, and CPU and memory request/limit flags. Customer OAuth may use server defaults, but it rejects a non-default template, every named volume, and CPU or memory overrides. Low-level resource overrides are operator-only.
