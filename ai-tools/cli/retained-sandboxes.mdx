...
The 0.6.2 help also exposes template, named-volume, and CPU and memory request/limit flags. Customer OAuth may use server defaults, but it rejects a non-default template, every named volume, and CPU or memory overrides. Low-level resource overrides are operator-only.
...
Show one unbound/operator sandbox's current state:
...
In 0.6.2, `status` cannot address an environment-bound customer sandbox.

## `aient sandbox wait`

Wait until an unbound/operator sandbox becomes ready:
...
In 0.6.2, `wait` cannot address an environment-bound customer sandbox.
...
Open a new interactive PTY shell in an unbound/operator sandbox:

```sh
aient sandbox shell laptop-offload
```

Every invocation opens a new shell. Shell reattachment, history replay, and capability-bearing environment or repository flags are not available in 0.6.2, so `shell` cannot address an environment-bound customer sandbox. Use `sandbox exec --environment ...` for authorised non-interactive operations.

## `aient sandbox logs`

Read a bounded recent unbound/operator sandbox log snapshot:
...
In 0.6.2, `logs` cannot address an environment-bound customer sandbox.
