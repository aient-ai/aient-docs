...
Customer environments own resource sizing. Customer OAuth rejects non-default templates, CPU or memory requests and limits, and every named volume even though their low-level flags are visible in 0.6.2 help. They are not customer sizing controls.
...
The default remote workspace is `/workspace/repo`. In 0.6.2, `sync` cannot carry the environment and repository selectors needed to reopen an environment-bound customer sandbox created by `run --keep`. Start a fresh `run --keep` to transfer changed customer workspace state.
