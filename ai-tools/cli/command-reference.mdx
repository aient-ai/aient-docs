...
<Warning>
  In 0.6.2, an environment-bound customer sandbox can be reused only through selector-bearing `exec` and `execution` commands, found with `list --environment`, and removed by its owner with `delete`. `sync`, `files`, `shell`, `status`, `wait`, and `logs` are unbound/operator-only for that sandbox. Start a fresh `run --keep` to transfer changed customer workspace state.
</Warning>
