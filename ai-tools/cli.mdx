...
The `aient` command runs local development work in isolated remote sandboxes. This guide documents Aient CLI 0.6.2, available as a static binary for macOS and Linux on Intel and Arm.
...
In 0.6.2, an environment-bound customer sandbox can be reused only through selector-bearing `sandbox exec` and `sandbox execution` commands, found with `sandbox list --environment`, and removed by its owner with `sandbox delete`. `sandbox sync`, `files`, `shell`, `status`, `wait`, and `logs` cannot yet reopen that bound sandbox. To transfer changed local state, delete it and start a fresh `sandbox run --keep`; never fall back to operator credentials.

The lower-level `sandbox create`, `sync`, and selector-less `exec` loop is an unbound/operator workflow. It does not mount a customer's environment or repository capabilities.

## Current preview boundary

The commands documented in this section are the surface shipped in Aient CLI 0.6.2.
...
- Public port exposure and sandbox size presets are not yet available.
- Non-default templates, CPU or memory overrides, named volumes, and the Docker lifecycle helper are operator controls, not customer sizing features.
