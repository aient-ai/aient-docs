...
The `aient` command runs local development work in isolated remote sandboxes. This guide documents Aient CLI 0.8.2, available as a static binary for macOS and Linux on Intel and Arm.
...
In 0.8.2, repeat `--environment` when using `status`, `wait`, or `logs` with an environment-bound customer sandbox. Selector-bearing `sandbox exec` and `sandbox execution` commands also require their original environment and repository selectors. `sandbox sync`, `files`, and `shell` still cannot reopen that bound sandbox. To transfer changed local state, delete it and start a fresh `sandbox run --keep`.
