...
The commands documented in this section are available in Aient CLI 0.10.15.
...
- Workspace-backed `sandbox run` and `sandbox sync` retain an immutable local operation receipt after foreground loss. Use `sandbox workspace status`, `resume`, or `discard` against that exact receipt instead of starting a replacement publication.
- `auth exec` gives a trusted local task a renewable access-only credential without passing through the selected refresh-backed profile. It is credential minimisation, not a local process sandbox.
