...
Every ordinary CLI process starts one fresh execution. Recovery inside that process reuses the exact execution ID and never redispatches the command; running the same command in another process starts another execution. `sandbox run --json` and `sandbox exec --json` emit `execution_started` with `executionId` and `outputCursor` after authoritative admission and before any command output.
...
The commands documented in this section are the surface shipped in Aient CLI 0.11.8.
