...
Aient webhooks deliver tenant-selectable lifecycle events as CloudEvents structured JSON. Verify signatures, process events idempotently with the CloudEvents (`source`, `id`) tuple, and return success only after durable acceptance.

## 16. What do I use the MCP for?

Use the Aient MCP when an AI assistant or coding agent needs live Aient context inside tools such as Cursor or Claude Code. It can read telemetry and problem context, and with onboarding/write scope it can report activation status, submit supported onboarding steps, and retry repository discovery. Keep payment, integration approval, and production-change authority on the normal consent and review surfaces.
...
Yes, provided you choose the right setup path. An autonomous wallet agent can discover Aient, pay with x402, create a wallet-backed organisation, and request remediation directly — see [Agentic x402 onboarding](./agentic-x402). An AI assistant can help a human team prepare setup safely — see [Agent-assisted setup](./agent-commerce) for what an assistant can and cannot do.

## 21. How does metered usage appear in Billing Settings?

Billing Settings separates usage recorded for the current period, estimated usage that has not reached an invoice, and provider invoices and payments. Stripe can issue an interim invoice when outstanding metered usage reaches your subscription's threshold; remaining metered usage is billed at the end of the cycle. A remediation is not charged as an individual purchase.
