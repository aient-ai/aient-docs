...
Aient is an AI operations layer for software teams. It ingests enough telemetry to detect when something goes wrong, groups related signals into a problem, explains what changed, and can drive remediation through Slack, GitHub, webhooks, and agent tools.

## 2. How can Aient know what goes wrong?

Aient learns from the signals your applications emit: traces, logs, exceptions, browser errors, release metadata, and source maps. OpenTelemetry gives Aient the runtime context; source maps and commit metadata connect minified browser frames back to source; integrations such as GitHub and Slack give Aient the collaboration context needed to route and explain the issue.

## 3. What signals does Aient ingest?

The public integration surface accepts OTLP HTTP traces and logs. The Aient SDKs configure the right endpoints, auth headers, service names, and release metadata for Node.js and browser apps. Webhook events, Slack, GitHub, and Linear integrations add lifecycle and collaboration context around the problems Aient creates.

## 4. How do I add Aient to my services?

Install `@aient/otel` in server services and `@aient/otel-browser` in browser apps, set an Aient publishable environment key, and pass stable `serviceName` plus release metadata. For browser apps, upload source maps from CI with `@aient/sourcemaps` using the same service name and commit SHA.
...
Aient can investigate and prepare remediation work, including GitHub pull requests, when the integrations and policies allow it. It should not be treated as an auto-merge system. Humans keep review and merge authority unless you deliberately build a stronger approval workflow around it.
...
Yes, if we make the agent fill out the same setup intent a human fills out today and send payment through Stripe. Aient still needs a provisioning API that turns that intent into a safe onboarding run: plan selection, Stripe checkout or subscription state, GitHub repository choice, Slack/GitHub consent links, environment keys, and status tracking. See [Agent Purchase And Configuration](./agent-commerce).
