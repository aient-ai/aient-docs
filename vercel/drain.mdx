...
- A Vercel plan tier that includes **Drains**. <Tooltip tip="Verify against your Vercel plan before you start — Drains availability and the custom-endpoint option depend on the plan tier.">Plan availability — verify.</Tooltip>
- An Aient account with an environment to receive the data.
- The per-environment **Drain URL** and **Signature Verification Secret** from Aient
  (see [Get your Drain URL and secret](#get-your-drain-url-and-secret)).

## Get your Drain URL and secret

<Note>
  **Draft — verify after the onboarding/settings screen ships.** The exact location of
  the Drain URL and secret in the Aient app, and the final URL host and path, are being
  implemented in parallel. The model below matches the plan; confirm the specifics once
  the feature lands.
</Note>

In Aient, open the environment you want Vercel data to land in and find its **Vercel
Drain** settings. Aient gives you, per environment and per data type:

- A **Drain URL** — a per-organisation, per-data-type endpoint. It looks like:

  ```txt
  https://ingest.aient.ai/drains/vercel/<your-drain-token>/logs
  https://ingest.aient.ai/drains/vercel/<your-drain-token>/traces
  ```
...
- A **Signature Verification Secret** — a per-organisation HMAC secret you paste into
  Vercel's "Signature Verification Secret" field. Aient uses it to verify that every
  Drain request genuinely came from your Vercel project before accepting the data.

Copy both before you start the Vercel steps below.
...
1. **Telemetry arrives.** Logs and traces land in Aient, stamped to your organisation and
   environment. The Drain settings show when data was last received.
...
<Tip>
  Want minified Vercel stack traces to resolve to original source? Make sure your
  deployment's commit SHA reaches Aient and [upload source maps](/source-maps/upload) for
  that build. <Tooltip tip="Vercel's log schema does not include the git commit SHA directly; how Aient derives it for Drain-ingested logs is being finalised. Symbolication degrades gracefully to raw frames when the SHA is absent.">Commit SHA on Drain logs — verify after merge.</Tooltip>
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Why does Aient verify a signature?">
    ...
    proves each request actually came from your Vercel project. Aient computes an HMAC
    over the raw request body and rejects requests that don't match — so a leaked URL
    alone can't be used to inject telemetry.
  </Accordion>
