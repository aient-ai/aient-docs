...
| --- | --- | --- | --- |
| Obfuscated Dart AOT on Android | `app.android-<arch>.symbols` | Yes | Yes |
...
The uploader does not proactively print configuration. When an upload fails, it
does not redact, truncate, or rewrite caught failure evidence: CI receives the
original exception/message and stack so the failure can be diagnosed. Keep
credentials out of command arguments and user-supplied exception values; CI log
hygiene remains the workflow owner's responsibility.
...
and uploader, and retain both `build/debug-info/**` and
`build/ios/archive/Runner.xcarchive/dSYMs/**` before publishing that same IPA to
App Store Connect.
